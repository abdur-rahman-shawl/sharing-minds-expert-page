        ...
        <div className="p-6 space-y-6">
            ...
            <div className="bg-gradient-to-r from-amber-500/10 via-amber-500/5 to-transparent rounded-xl border border-amber-500/20 p-4 flex items-center gap-3">
                ...
                <div className="flex-1">
                    <p className="text-sm font-medium text-amber-600 dark:text-amber-300">Analytics is coming soon</p>
                    <p className="text-xs text-gray-500 dark:text-slate-400 mt-0.5">This is a preview of your analytics dashboard. All data shown is placeholder.</p>
                </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                ...
                <div className="bg-gray-50 dark:bg-slate-900/60 rounded-xl border border-gray-200 dark:border-slate-800 p-5">
                    <div className="flex items-center justify-between mb-5">
                        <h3 className="text-sm font-semibold text-gray-700 dark:text-slate-300">Profile Views</h3>
                        <span className="text-[10px] text-gray-400 dark:text-slate-600 px-2 py-0.5 bg-gray-100 dark:bg-slate-800 rounded-md">This Week</span>
                    </div>
                    <div className="flex items-end gap-3 h-32 px-2">
                        {WEEKLY_DATA.map((item) => (
                            <div key={item.day} className="flex-1 flex flex-col items-center gap-1.5">
                                ...
                                />
                                <span className="text-[9px] text-gray-400 dark:text-slate-600">{item.day}</span>
                            </div>
                        ))}
                    </div>
                </div>
                ...
                <div className="bg-gray-50 dark:bg-slate-900/60 rounded-xl border border-gray-200 dark:border-slate-800 p-5">
                    <h3 className="text-sm font-semibold text-gray-700 dark:text-slate-300 mb-5">Engagement Breakdown</h3>
                    <div className="flex items-center justify-center h-32">
                        <div className="relative">
                            <svg className="h-28 w-28" viewBox="0 0 100 100">
                                <circle cx="50" cy="50" r="40" fill="none" className="stroke-gray-200 dark:stroke-slate-800" strokeWidth="12" />
                                <circle cx="50" cy="50" r="40" fill="none" className="stroke-gray-300 dark:stroke-slate-700" strokeWidth="12"
                                    strokeDasharray="60 191" strokeDashoffset="0" strokeLinecap="round" opacity="0.3" />
                                <circle cx="50" cy="50" r="40" fill="none" className="stroke-gray-400 dark:stroke-slate-600" strokeWidth="12"
                                    strokeDasharray="40 211" strokeDashoffset="-60" strokeLinecap="round" opacity="0.2" />
                                <circle cx="50" cy="50" r="40" fill="none" className="stroke-gray-500 dark:stroke-slate-500" strokeWidth="12"
                                    strokeDasharray="30 221" strokeDashoffset="-100" strokeLinecap="round" opacity="0.15" />
                            </svg>
                            <div className="absolute inset-0 flex items-center justify-center">
                                <div className="text-center">
                                    <p className="text-lg font-bold text-gray-400 dark:text-slate-600">0</p>
                                    <p className="text-[9px] text-gray-400 dark:text-slate-600">Total</p>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div className="flex justify-center gap-4 mt-3">
                        <Legend color="bg-gray-400/30 dark:bg-slate-500/30" label="Sessions" />
                        <Legend color="bg-gray-500/30 dark:bg-slate-600/30" label="Messages" />
                        <Legend color="bg-gray-600/30 dark:bg-slate-700/30" label="Content" />
                    </div>
                </div>
            </div>
            ...
            {/* Top skills */}
            <div className="bg-gray-50 dark:bg-slate-900/60 rounded-xl border border-gray-200 dark:border-slate-800 p-5">
                <h3 className="text-sm font-semibold text-gray-700 dark:text-slate-300 mb-4">Top Skills by Demand</h3>
                <div className="space-y-3">
                    {['Skill 1', 'Skill 2', 'Skill 3', 'Skill 4'].map((skill, i) => (
                        <div key={skill} className="flex items-center gap-3">
                            <span className="text-xs text-gray-400 dark:text-slate-600 w-16">{skill}</span>
                            <div className="flex-1 h-2 bg-gray-200 dark:bg-slate-800 rounded-full overflow-hidden">
                                <div
                                    className="h-full bg-indigo-500/20 dark:bg-indigo-600/20 rounded-full"
                                    style={{ width: `${[45, 35, 25, 15][i]}%` }}
                                />
                            </div>
                            <span className="text-[10px] text-gray-400 dark:text-slate-600 w-6 text-right">0</span>
                        </div>
                    ))}
                </div>
            </div>
...
function MetricCard({ icon: Icon, label, value, trend, color }: {
    ...
    return (
        <div className="bg-gray-50 dark:bg-slate-900/60 rounded-xl border border-gray-200 dark:border-slate-800 p-4">
            ...
            <p className="text-xl font-bold text-gray-400 dark:text-slate-500">{value}</p>
