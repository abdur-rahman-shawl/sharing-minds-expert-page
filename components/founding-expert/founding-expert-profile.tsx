...
import { foundingExpertProfile } from "./founding-expert-data"

export function FoundingExpertProfile() {
  return (
    <article className="relative overflow-hidden rounded-[22px] border border-[#d59b36]/80 bg-white/72 p-4 shadow-[0_18px_42px_rgba(113,72,14,0.13)] backdrop-blur-sm sm:p-5 lg:h-[clamp(214px,27vh,250px)] lg:p-[clamp(14px,1.35vw,20px)] 2xl:flex 2xl:h-[clamp(214px,27vh,280px)] 2xl:flex-col 2xl:justify-center">
      ...
      <div className="relative grid gap-4 sm:grid-cols-[122px_minmax(0,1fr)_152px] sm:items-center lg:grid-cols-[clamp(102px,9vw,128px)_minmax(0,1fr)_clamp(126px,12vw,160px)]">
        <div className="relative mx-auto h-[112px] w-[112px] sm:h-[122px] sm:w-[122px] lg:h-[clamp(102px,9vw,128px)] lg:w-[clamp(102px,9vw,128px)]">
          <div className="absolute inset-0 rounded-full bg-[conic-gradient(from_35deg,#f0c45a,#9b620e,#f7df91,#b57714,#f0c45a)] p-[3px] shadow-[0_12px_24px_rgba(87,55,7,0.22)]">
            <div className="relative h-full w-full overflow-hidden rounded-full border-[5px] border-[#092348] bg-[#102f59]">
              <Image
                src={foundingExpertProfile.image}
                alt="Representative SharingMinds founding expert"
                fill
                sizes="128px"
        ...
        <div className="min-w-0 text-center sm:text-left">
          <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-[#aa6b12]">
            {foundingExpertProfile.eyebrow}
          </p>
          <h3 className="mt-1 text-[clamp(22px,2vw,30px)] font-semibold leading-tight tracking-[-0.025em] text-[#0a2148]">
            {foundingExpertProfile.title}
          </h3>
          <p className="mt-1 text-[12px] font-medium leading-snug text-[#263b57] lg:text-[13px]">
            {foundingExpertProfile.disciplines}
          </p>

          <ul className="mt-3 grid gap-1.5 text-left sm:grid-cols-3 lg:grid-cols-1 xl:grid-cols-[0.7fr_1.45fr_1.2fr]">
            {foundingExpertProfile.attributes.map(({ label, icon: Icon }) => (
              <li
                key={label}
      ...
      <ul className="relative mt-4 flex flex-wrap justify-center gap-2 sm:justify-start lg:mt-3">
        {foundingExpertProfile.specialisms.map((specialism) => (
          <li
            key={specialism}
