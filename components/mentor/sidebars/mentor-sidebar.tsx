...
interface MentorSidebarProps {
    activeSection?: string // Made optional/deprecated as we use usePathname
    onSectionChange?: (section: string) => void // Deprecated
}

export function MentorSidebar({ activeSection: propActiveSection, onSectionChange }: MentorSidebarProps) {
    ...
    const getCurrentSection = () => {
        if (pathname === '/dashboard/mentor') return 'dashboard'

        // Extract section from /dashboard/mentor/[section]
        const match = pathname?.match(/\/dashboard\/mentor\/([^/]+)/)
    ...
    const handleNavigation = (key: string) => {
        router.push(`/dashboard/mentor/${key === 'dashboard' ? '' : key}`);
    };
    ...
    const profileImage = mentorProfile?.profileImageUrl || session?.user?.image || undefined;
    ...
    return (
        <Sidebar className="bg-white dark:bg-gray-900 border-r border-gray-200 dark:border-gray-800 mt-16">
            ...
            <SidebarFooter className="p-4 border-t border-gray-100 dark:border-gray-800">
                <Button
                    className="w-full bg-blue-500 hover:bg-blue-600 text-white gap-2 h-10 rounded-lg font-medium text-sm transition-colors duration-200"
