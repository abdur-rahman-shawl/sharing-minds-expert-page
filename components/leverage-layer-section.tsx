...
export function LeverageLayerSection() {
  return (
    <section
      ...
      className="relative scroll-mt-[104px] overflow-hidden bg-[#03172c] px-5 py-10 text-white sm:px-8 sm:py-12 lg:px-12"
    >
      ...
      <div className="relative mx-auto grid w-full max-w-[1380px] gap-12 lg:grid-cols-[250px_minmax(0,1fr)] lg:items-center lg:gap-10">
        <div className="max-w-[290px]">
          <div className="mb-6 flex items-end gap-3">
            <span className="font-serif text-[33px] leading-none text-[#e0aa5b]">03</span>
            <span className="pb-0.5 text-[8px] font-semibold uppercase tracking-[0.1em] text-[#5e9bd8]">
              A Strategic Leverage Layer
            </span>
          </div>

          <h2
            id="leverage-layer-title"
            className="font-serif text-[clamp(34px,2.6vw,42px)] font-normal leading-[1.02] tracking-[-0.025em] text-[#f4f1eb]"
          >
            A Strategic
            <span className="block">Leverage Layer.</span>
          </h2>

          <p className="mt-5 text-[12px] leading-[1.7] text-[#d5dce5]">
          ...
          <ul className="mt-5 space-y-2.5">
            {leveragePoints.map(({ label, icon: Icon }) => (
              <li key={label} className="flex items-center gap-3 text-[12px] text-[#e5eaf0]">
                <Icon className="h-4 w-4 shrink-0 stroke-[1.6] text-[#e3a954]" />
                {label}
              </li>
            ))}
          </ul>
        ...
        <div className="relative">
          <div className="pointer-events-none absolute left-[5%] right-[5%] top-[84px] hidden h-[118px] lg:block">
          ...
          <ol className="relative grid gap-9 sm:grid-cols-2 lg:grid-cols-5 lg:gap-4">
            {stages.map(({ number, title, description, image, icon: Icon }, index) => (
              <li key={number} className="relative flex flex-col items-center text-center">
                <div className="relative mb-4 aspect-square w-full max-w-[178px]">
                  ...
                  <span className="absolute left-1/2 top-0 z-10 flex h-11 w-11 -translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-full border border-[#bf8d50] bg-[#061a30] shadow-[0_0_16px_rgba(4,18,35,0.85)]">
                    <Icon className="h-5 w-5 stroke-[1.5] text-[#e2ab5d]" />
                  </span>
                </div>

                <span className="font-serif text-[20px] leading-none text-[#e6b267]">{number}</span>
                <h3 className="mt-1 text-[13px] font-semibold text-[#f4f5f7]">{title}</h3>
                <p className="mt-2 max-w-[190px] text-[10px] leading-[1.55] text-[#c8d1dc]">
                  {description}
                </p>
              </li>
            ))}
